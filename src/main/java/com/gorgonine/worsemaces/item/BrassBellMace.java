...
public class BrassBellMace extends Item {
    ...
    public BrassBellMace(Settings settings) {
        super(settings);
    }
    ...
    public void postHit(ItemStack stack, LivingEntity target, LivingEntity attacker) {
        if (shouldDealAdditionalDamage(attacker)) {
            ...
            serverWorld.playSound((Entity)null, target.getX(), target.getY(), target.getZ(), ModSounds.bell_hit, target.getSoundCategory(), 2.0F, random.nextFloat(2.0F));
            serverWorld.playSound(
                    attacker, // Player - if non-null, will play sound for every nearby player *except* the specified player
    ...
    public double getFormulaMinimum(double playerFallDistance){
        return 3.0 * playerFallDistance/2;
    }
    public double getFormulaMiddle(double playerFallDistance){
        return 8.0 + 1.5 * (playerFallDistance/2 - MAX_MIN_FALL_DISTANCE);
    }
    public double getFormulaMax(double playerFallDistance){
        return 14.0 + playerFallDistance/2 - MIDDLE_FALL_DISTANCE;
    }
