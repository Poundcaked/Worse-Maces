...
public class PufferfishMace extends Item {
    ...
    public PufferfishMace(Settings settings) {
        super(settings);
    }
    ...
    public void postHit(ItemStack stack, LivingEntity target, LivingEntity attacker) {
        ...
        if (shouldDealAdditionalDamage(attacker)) {
            ...
            areaEffectCloudEntity.setDuration(15*20);
            ...
            if (target.isOnGround()) {
                ...
                SoundEvent soundEvent = attacker.fallDistance > HEAVY_SMASH_SOUND_FALL_DISTANCE_THRESHOLD ? SoundEvents.ENTITY_PUFFER_FISH_BLOW_OUT : SoundEvents.ENTITY_PUFFER_FISH_BLOW_UP;
                ...
                serverWorld.playSound((Entity)null, attacker.getX(), attacker.getY(), attacker.getZ(), SoundEvents.ENTITY_PUFFER_FISH_STING, attacker.getSoundCategory(), 1.0F, 1.0F);
            }
        ...
        if(!target.isAlive()){
            ...
            target.getWorld().playSound(null,target.getBlockPos(), ModSounds.pufferfish_noise, SoundCategory.PLAYERS,0.9F,random.nextFloat(0.8F,1.4F));
        }
    ...
    public double getFormulaMinimum(double playerFallDistance){
        return 3.0 * playerFallDistance;
    }
    public double getFormulaMiddle(double playerFallDistance){
        return 8.0 + 1.6 * (playerFallDistance - MAX_MIN_FALL_DISTANCE);
    }
    public double getFormulaMax(double playerFallDistance){
        return 17.0 + playerFallDistance - MIDDLE_FALL_DISTANCE;
    }
    ...
    static void knockbackNearbyEntities(World world, Entity attacker, Entity attacked) {
        ...
        world.getEntitiesByClass(LivingEntity.class, attacked.getBoundingBox().expand(KNOCKBACK_RANGE), getKnockbackPredicate(attacker, attacked)).forEach((entity) -> {
            ...
            var poison = new StatusEffectInstance(StatusEffects.POISON, 6 * 20, 1, false, true, true); //get oozing
